    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-dark-brown/80 backdrop-blur-md" onClick={onClose}>
      <div className="bg-cream w-full max-w-5xl rounded-2xl overflow-hidden shadow-2xl relative flex flex-col md:flex-row max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="w-full md:w-1/2 p-6 flex flex-col gap-4 bg-white/50 border-r border-dark-brown/5">
          {/* ФОТО: object-contain + scale-90 (не обрізаємо, додаємо "повітря") */}
          <div className="relative aspect-square rounded-xl overflow-hidden bg-white shadow-inner flex items-center justify-center">
            <Image 
              src={mainImage} 
              alt={bag.name} 
              fill 
              // object-contain гарантує видимість всієї сумки. scale-90 додає відступ.
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {bag.images.map((img, idx) => (
              <button 
                ...
                // Прев'ю теж object-contain і scale-90
                className={`relative w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-white border-2 transition-all flex items-center justify-center ${mainImage === img ? 'border-burgundy scale-95' : 'border-transparent opacity-70'}`}
              >
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 overflow-y-auto bg-white/30 flex flex-col">
          ...
          <div className="space-y-6 mb-10 flex-grow">
            <div>
              ...
              <p className="font-sans text-sm md:text-base text-dark-brown leading-relaxed font-medium">{bag.description}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-burgundy/10 pt-6">
              ...
              <div>
                {/* ЗМІНЕНО: Заголовок "Колекція" замість "Матеріали" */}
          ...
          <a href={telegramLink} target="_blank" className="block w-full py-4 mt-auto bg-burgundy text-cream text-center font-bold uppercase tracking-widest hover:bg-dark-brown transition-all rounded-sm shadow-lg active:scale-95">
            Купити зараз
          </a>
        </div>
      </div>
